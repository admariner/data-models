...
CREATE OR REPLACE TABLE {{.scratch_schema}}.link_clicks{{.entropy}} AS(
  WITH click_count AS(
    SELECT
      ev.page_view_id,
      COUNT(DISTINCT ev.event_id) AS link_clicks,
    ...
    SELECT
      ...
      FIRST_VALUE(ev.unstruct_event_com_snowplowanalytics_snowplow_link_click_1_0_1.target_url) OVER(PARTITION BY ev.page_view_id
         ORDER BY ev.derived_tstamp desc
         ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS first_link_clicked,

      LAST_VALUE(ev.unstruct_event_com_snowplowanalytics_snowplow_link_click_1_0_1.target_url) OVER(PARTITION BY ev.page_view_id
         ORDER BY ev.derived_tstamp desc
